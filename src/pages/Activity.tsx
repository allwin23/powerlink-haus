...
import { AlertCircle, Loader2, ArrowDownLeft, ArrowUpRight } from 'lucide-react';
import type { TimelineColor } from 'src/types';
...
const TimelineIcon = ({
  ...
  const colorClasses = {
    primary: 'bg-[#77DD77] text-black',
    secondary: 'bg-secondary text-secondary-foreground',
    muted: 'bg-muted text-muted-foreground',
    accent: 'bg-accent text-accent-foreground',
    destructive: 'bg-[#FF6961] text-white',
  };
...
export default function Activity() {
  const transactions = [
    {
      type: 'buy',
      counterparty: '0x' + Array.from({ length: 40 }, () => Math.floor(Math.random() * 16).toString(16)).join(''),
      timestamp: '2024-03-07T14:30',
      energy: '10 kWh',
      price: '$5.00',
    },
    {
      ...
      <Timeline>
        {transactions.map((transaction, index) => (
          <TimelineItem
            ...
            title={`${transaction.type === 'buy' ? 'Bought' : 'Sold'} Energy`}
            description={`From/To: ${transaction.counterparty} - ${transaction.energy} at ${transaction.price}`}
            iconColor={transaction.type === 'buy' ? 'destructive' : 'primary'}
            icon={transaction.type === 'buy' ? <ArrowDownLeft /> : <ArrowUpRight />}
